...
export default function ChunkedDownloader() {
  ...
  async function handleDownload() {
    setStatus("starting");
    if (!owner || !metadataCid) return alert("owner and metadataCid required");
    ...
    try {
      ...
      // ask MetaMask to decrypt the AES key
      const aesKeyBase64 = await (ethDecrypt ? ethDecrypt(cipherJson, account) : window.ethereum.request({ method: "eth_decrypt", params: [cipherJson, account] }));
